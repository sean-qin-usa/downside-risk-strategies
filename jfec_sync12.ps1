...
"=== jfec_sync12 $(Get-Date -Format o) ===" | Out-File $log -Encoding utf8
Set-Location $PROJ
...
git commit -m "Wave-8: 93-look Holm adjustment (7 survivors, PJM reclassified as artifact); strict audit renamed job_fz_strict_calibration; one 100-word abstract in both builds; FRTB-aligned wording; CAViaR pooled-rejection rhetoric removed; em-dash de-AI pass 220 to 119" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>" -m "Claude-Session: https://claude.ai/code/session_01PaysugvHD2tgCJvXLSscWa" 2>&1 | Select-Object -First 2 | Out-File $log -Append -Encoding utf8
...
git add -A 2>&1 | Out-Null
git commit -m "Cross-universe Holm table OA.3; strict-calibration artifacts under canonical names; unified 100-word abstract; prose pass" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>" -m "Claude-Session: https://claude.ai/code/session_01PaysugvHD2tgCJvXLSscWa" 2>&1 | Select-Object -First 2 | Out-File $log -Append -Encoding utf8
