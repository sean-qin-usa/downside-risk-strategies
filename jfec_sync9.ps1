...
"=== jfec_sync9 $(Get-Date -Format o) ===" | Out-File $log -Encoding utf8
Set-Location $PROJ
...
git commit -m "Wave-6: h-day boundary purge (conclusions unchanged); ten-day model relabeled as direct extension; sign convention unified; purged horizon curve; README states which file is the paper" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>" -m "Claude-Session: https://claude.ai/code/session_01PaysugvHD2tgCJvXLSscWa" 2>&1 | Select-Object -First 2 | Out-File $log -Append -Encoding utf8
...
git add -A 2>&1 | Out-Null
git commit -m "Boundary-purged multi-day artifacts; purged horizon curve; new README (which file is the paper, script-to-result map); root tidied into code/ and results/" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>" -m "Claude-Session: https://claude.ai/code/session_01PaysugvHD2tgCJvXLSscWa" 2>&1 | Select-Object -First 2 | Out-File $log -Append -Encoding utf8
