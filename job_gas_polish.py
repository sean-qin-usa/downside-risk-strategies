# job_gas_polish.py -- GAS benchmark POLISH: multi-start, higher iteration budget,
# explicit convergence audit. Adversarial review: "zero fit failures only means no
# Python exception; res.success is never checked; single start, 80 iterations."
# This job re-estimates the one-factor FZ-score-driven GAS per name with THREE starts
# x maxiter 300, records scipy convergence status, keeps the best objective, and
# re-scores the FZ0 comparison vs the engine accuracy layer on the same panel.
...
for (pn,y,dts,sp) in gasrows:
    ...
    for a in ALPHAS:
        ...
        best=None
        for x0 in ([0.0,0.95,0.02],[0.0,0.90,0.05],[0.01,0.85,0.10]):
            try:
                res=optimize.minimize(obj,x0=x0,method='Nelder-Mead',
...
lg("GAS fitted multi-start (%d exception-fails) %.0fs"%(gfail,time.time()-t0))
for a in ALPHAS: lg("  alpha %s: convergence-flag rate %.3f over %d names"%(a,float(np.mean(CONV[a])) if CONV[a] else -1,len(CONV[a])))
def nw_t(x,l=10):
...
OUT={'note':'GAS POLISH: one-factor FZ-score-driven benchmark with 3 starts x maxiter 300, best objective kept, scipy convergence flag recorded per name. DM_t>0 = row model worse than engine. Compare gas_pzc rows to the single-start run in fz_fullpanel_results.json.',
     'gas_convergence_rate':{str(a):round(float(np.mean(CONV[a])),3) for a in ALPHAS if CONV[a]},
...
json.dump(OUT,open(os.path.join(P,"gas_polish_results.json"),"w"),indent=2)
lg("GASPOLISHDONE %.0fs"%(time.time()-t0))
