...
## R48 (2026-09-22): ten-day diagnostics run (job_tenday_diag.py, predictions in the script header)

Result file results/paper/tenday_diag_results.json. Same purged pipeline as the paper's ten-day numbers; h = 10; 4,000 simulated paths per row; NW(10) per-date DM.

Design era (150 names, 164,850 test rows, 2020-2024 test window): direct edge over sqrt(h)-GARCH-t +1.41% (DM 6.8; paper's 1.4% reproduced); over iterated-variance GARCH-t +0.79% (DM 4.0); over simulated GARCH-t paths +0.70% (DM 3.5); over bootstrapped filtered paths +0.51% (DM 2.5; 1.1 on non-overlapping dates). Iterated ten-day variance exceeds h times the one-day variance by 6-11% in every test year, so the sqrt(h) rule under-scales; it over-breaches (1.24% at 1%, 3.3% at 2.5%) while the iterated benchmarks and the direct model sit near nominal.
Holdout (200 names, 278,808 rows, mid-2008 to 2013): every pairwise DM within 0.3 (direct vs sqrt(h) +0.52%, DM 0.09; vs iterated benchmarks +0.5 to +0.8%, DM 0.14 to 0.25); term-structure ratio 0.99 in 2008-2009, 1.02-1.04 after. 2008 one-percent coverage is 3.8-4.5% for every model including the direct one (4.1%); the calibration reversal in the text is the ES statement of stress_es_results.json, and VaR coverage does not separate the models.
Predictions: P1 (benchmark is the story) confirmed in effect and wrong in sign, since the era's failure of sqrt(h) is under-scaling from a low starting variance, not over-scaling after spikes; P2 (direct tail fails to extrapolate in 2008 at the VaR level) not confirmed; P3 (overlap inflation) not confirmed, the non-overlapping DMs fall by less than the sample loss alone would imply.

Text changes:
1. Section 5.2, after "(Online Appendix Figure~OA.3)." Added: "About half of that edge is the scaling rule itself. The $\sqrt{h}$ rule holds the one-day variance flat across the window, while the fitted GARCH variance mean-reverts toward an unconditional level that sits $6$--$11\%$ above $h$ times the one-day variance in every year of the 2020--2024 test window; against a GARCH-$t$ whose variance is iterated through the window the direct model's edge is $0.8\%$ (DM 4.0), against simulated GARCH-$t$ paths $0.7\%$ (DM 3.5), and against bootstrapped filtered paths $0.5\%$ (DM 2.5), on the same rows."
   Same paragraph, holdout sentence. Before: "does not survive the pre-committed 2000--2013 rerun: out of era the two methods tie on accuracy (direct edge $+0.49\%$, DM 0.07) and the calibration verdict reverses". After: "does not survive the pre-committed 2000--2013 rerun, where the fitted term structure is flat (iterated ten-day variance within $2\%$ of $h$ times the one-day variance in 2008--2009): out of era the two methods tie on accuracy (direct edge $+0.49\%$, DM 0.07), as do the iterated benchmarks (DM at most 0.3), and the calibration verdict reverses".
2. Conclusion, the R47 sentence. Before: "it beats $\sqrt{h}$-scaled GARCH on the 2014--2024 panel and only ties it on the pre-committed 2000--2013 rerun". After: "it beats $\sqrt{h}$-scaled GARCH on the 2014--2024 panel, with about half of that edge surviving an iterated GARCH benchmark, and only ties both on the pre-committed 2000--2013 rerun".
3. Online Appendix Figure OA.3 caption. Before (R47): "the aggregate tail thins as days are summed while $\sqrt{h}$ scaling carries the one-day shape forward unchanged, so the scaling error grows with the horizon, from 0.45% at h=1 to 2.1% at h=20." After: "the $\sqrt{h}$ rule holds the one-day variance flat across the window while the fitted GARCH variance mean-reverts, and the gap grows with the horizon, from 0.45% at h=1 to 2.1% at h=20; at h=10 an iterated-variance GARCH-$t$ benchmark removes about half of it (main text, ten-day subsection)." Reason: the run attributes 0.6 of the 1.4 points to the variance term structure and under 0.1 to shape aggregation, so the R47 shape wording was also wrong.

## R49 (2026-09-24): whole-paper contradiction audit, factual fixes

Every change is a mislabelled or mismatched figure between two parts of the paper, checked against the result files. No abstract change.

1. Section 3, holdout replication of the composite. Before: "marginally above its kurtosis component ($+1.04\%$, DM 2.16), and". After: "marginally above its kurtosis component ($+1.04\%$, DM 2.16, on the residual-hybrid learner; the frozen returns-space learner of Section~\ref{sec:frontier} gives $+0.89\%$ at the same DM), and". Source: composite_holdout_results.json (residual-space learner) vs holdout_frontier_results.json (returns-space learner); Section 4.1 quoted the 0.89 without saying the learners differ.
2. Section 4.1, jump-robust decomposition. Before: "at four sigma, while the robust filter alone accounts for $+3.17\%$ (DM $8.2$) of the standard-GARCH gap in that decile." After: "at four sigma. The robust filter alone beats standard GARCH by $+3.17\%$ (DM $8.2$) in that decile, more than the estimator's $+2.98\%$, and the estimator rebuilt on the robust filter beats standard GARCH by $+3.51\%$ (DM $8.4$)." Source: frontier_robust_results.json K9 MECH 3.168/8.2 and ROBENGINE_vs_stdgarch 3.508/8.44; 3.17 cannot be "part of" a 2.98 gap.
3. Section 5.1, adaptive conformal shift. Before: "breach $1.83\%\to2.06\%$". After: "breach $1.63\%\to2.06\%$". Source: fz_aci_results.json; 1.83% is the gamma = 0.02 adaptive variant, 1.63% the static shift the sentence compares against.
4. Section 4.1, GARCH-EVT paragraph. Before: "shows no frontier." After: "shows no frontier of the estimator's size." (the same sentence reports +0.31%, DM 3.6 per name). Added after the per-name GARCH-EVT sentence: "GJR-GARCH-skew-$t$, which adds a leverage term to the filter, shows no frontier either (top-decile edge over GARCH-$t$ $+0.16\%$, DM 0.6), and the estimator beats it by $+2.32\%$ (DM 9.7) in the top decile." Source: bench_all_results.json; the intro's +2.32% (R46) had no body support.
5. Figure 2 caption. Before: "FHS (DM $4.9$)". After: "FHS (DM $4.9$ and $5.3$)". Source: pzc_taylor_results.json 2.5% fhs DM 5.27.
6. Section 5.1. Before: "while tying it at 2.5\% (DM 0.1)". After: "(DM 0.1 in that run)". The same paragraph quotes DM −0.1 from the same-rows rerun; both are correct for their runs.
7. Introduction. "(Section~\ref{sec:frontier})" after the annual-refit average now "(Section~\ref{sec:calendarleak})", where the walk-forward numbers are.
8. Conclusion. "(Section~\ref{sec:frtb})" after the jump-robust/realized-variance clause now "(Sections~\ref{sec:frontier} and \ref{sec:frtb})".
9. Section 5.2, holdout sentence. Before: "(direct edge $+0.49\%$, DM 0.07), as do the iterated benchmarks (DM at most 0.3), and the calibration". After: "(direct edge $+0.49\%$, DM 0.07); against the iterated benchmarks the point estimate is the same $0.5$--$0.8\%$ as in era, but with DM at most 0.3, because per-date loss differences through 2008 are far noisier; and the calibration". Conclusion clause adjusted to match ("the point estimate against the iterated benchmark is unchanged but within noise"). Source: tenday_diag_results.json, holdout edge_pct_direct_vs 0.52-0.81, DM 0.09-0.25.

Audit items left for Sean's decision (not edited): abstract and conclusion attribute day-one new-listing forecasts to the estimator whose FZ0 is reported, while Section 6 says only the characteristics-only amortized variant does that; Section 2.2 "repairs the 97.5% level that each model otherwise fails" is the pooled Kupiec, while Section 5.1 says the EVT tail passes the date-clustered test alone; Section 5.1 "residual-hybrid and GARCH-t are the two models that pass the 1% Kupiec test" is the 200-name date-bootstrapped test, Table 3's pooled p-values on 140 names disagree; intro "roughly ninety percent of asset-days indistinguishable" vs Romano-Wolf leaving decile 9 and the jump signal's bottom decile significant; "smaller but significant shape component" (jump-robust, full panel) vs "within noise" (realized-variance refit, large caps) both described as the same story; tie convention (|DM| < 1.645) vs "tie" at DM −1.8/−1.9 in the CAViaR and pooled-body sentences; intro "estimator and CAViaR both remain in the 90% MCS" while Table 3's MCS holds the tail-less residual-hybrid alone; 0.75% trees-vs-IQN figure not in Table 4; "five evaluation panels" omits the auxiliary panels; OptionMetrics listed in data availability but unused.

## R50 (2026-09-24): second-reader review of R49, verified items applied

Each item was checked against the tex and the result files before editing. Abstract unchanged.

Configuration labels (Online Appendix Table OA.10 gives pooled body +2.98% DM 10.48 and EVT-completed accuracy layer +2.48% DM 10.08 in the top kurtosis decile; the intro had mixed the two).
1. Introduction. Before: "(top-decile edge $+2.98\%$, DM 10.5, and $+2.32\%$, DM 9.7, against GJR-GARCH-skew-$t$ on the same rows)". After: "(top-decile edge $+2.98\%$, DM 10.5, for the pooled residual body without the EVT tail; the EVT-completed accuracy layer's edge on the same rows is $+2.48\%$, DM 10.1, against GARCH-$t$ and $+2.32\%$, DM 9.7, against GJR-GARCH-skew-$t$)".
2. Table 1 note, added: "The nonparametric forecast in this table is the pooled residual body of Stage 2 without the EVT tail; the EVT-completed accuracy layer of Section 5 carries $+2.48\%$ (DM 10.1) in the top kurtosis decile on the same rows (Online Appendix Table OA.10)."
3. Section 4.1: "and the estimator beats it by $+2.32\%$" -> "and the accuracy layer beats it by $+2.32\%$"; "Against that benchmark the top-decile edge falls" -> "Against that benchmark the pooled body's top-decile edge falls".
Tie convention (Section 3.3: two-sided 10%, |DM| < 1.645).
4. Section 4.1. Before: "the two tie in every region (top decile $-0.33\%$, DM $-1.8$; overall $-0.12\%$, DM $-1.6$, engine over CAViaR)". After: "the two are within a third of a percent in every region (top decile $-0.33\%$, DM $-1.8$, marginal under the convention of Section~\ref{sec:fwer}; overall $-0.12\%$, DM $-1.6$, engine over CAViaR)".
5. Section 5.1. Before: "lower still by a margin within noise (DM $-1.3$ and $-1.9$)". After: "lower still (DM $-1.3$ at 1\%, within noise, and $-1.9$ at 2.5\%, marginal under the convention of Section~\ref{sec:fwer})".
Claims restated at the strength the evidence carries.
6. Section 1.2: "the conventional EVT tail carries no frontier of its own" -> "carries a much smaller frontier ($+0.31\%$ per name, none with a pooled tail)".
7. Figure 1 caption: "survives an untouched era containing the 2008 crisis." -> "reappears, as a post-hoc check, in an era containing the 2008 crisis that no design stage used." (matches Section 3's statement that the composite was not the frozen rule).
8. Conclusion: "the flexible model cannot improve on it" -> "does not measurably improve on it".
9. Lemma 2: "(guaranteed by the rearrangement step)" -> "(the rearrangement step guarantees this for $Q^{z}_t$; a level-dependent shift $c_\tau$ requires rearranging the shifted curve again)".
10. Section 5.2: "the full tables are in the Online Appendix" -> "the benchmark table is Online Appendix Table~OA.13", and Table OA.13 added (edge, DM, non-overlap DM and coverage for the four ten-day benchmarks in both eras, with the overlap treatment stated in the note). Sample-flow table renumbered OA.14.
Wording (the reader's AI-tell list, applied where the sentence overstated).
11. "Fissler--Ziegler" -> "Fissler--Ziegel" (introduction). "so with no history it has nothing" -> "so it has no forecast until a history exists". "so a parametric-adjacent fix cannot absorb the edge" -> "so this reweighted-FHS specification does not absorb the edge". "The decisive standalone wins" -> "The largest standalone wins" (introduction and Section 4.2). OA.8: "Two mechanisms explain it, and both teach." -> "Two mechanisms explain it."; "static kurtosis blending is a second-order sin" -> "the regret from one fitted shape across both regimes is second order"; "cannot be an artifact of stationary fat tails being averaged over (that configuration is the one the simulation shows a fixed shape handles)" -> "is not what averaging stationary fat tails produces in this simulation (a fixed shape handles that configuration)".
Production.
12. OA Figure OA.2 caption: "Table~6 of the paper" -> "Table~3 of the paper". OA glossary column now wraps (was clipped at the right margin). OA.7 schematic: boxes respaced and arrow labels moved below the arrows (were overlapping the Stage 2 and 3 boxes). Proof of Lemma 2 moved before the bibliography. OA calendar paragraph: annual walk-forward "+2.09% at DM 4.43 ... (-0.18%, DM -0.52)" -> "+2.12% at DM 4.41 ... (-0.22%, DM -0.59)", the committed walkforward_hybrid_results.json values that the main text already quotes.
13. OA Figure OA.6: lower deciles were drawn at zero. job_composite_profiles.py reruns Table 1 with the full decile profile of each signal (composite_profiles_results.json; reproduces every Table 1 number exactly, including decile 9 at +0.46/+0.59 and the jump bottom decile +1.17, DM 5.5, which had no committed source before). The figure now plots the actual bulk values, all within ±0.2%.

Reader items not applied, for Sean: "distribution-free tail" in the abstract (the frontier headline is the pooled body, which is distribution-free; the FZ0 sentence is the EVT-completed layer, so the abstract describes two configurations); the day-one attribution in abstract and conclusion; OA.9 selling-tail-protection paragraph (scope); Francq-Zakoian wording in Section 2.2 (the t-QMLE scale consistency point); "well calibrated" against OA.5 McNeil-Frey p = 0.04 at 1%; how ES is formed under the conformal overlay (integral of the shifted or unshifted curve; needs a code check in fz_fullpanel/fz_aci before any formula changes); funding line wording; the claim that the introduction grew from 690 words is measured against a non-submitted ChatGPT build, not R32.

## R51 (2026-09-24): second-reader follow-up on R50, verified items

1. Section 2.2, after the ES display below equation (5). Added: "The overlay applies one scalar $c_{0.025}$ to the VaR node and to the ES integral at that level, so the shift sits outside the integral by construction; the accuracy layer sets $c_\tau\equiv0$ and the two coincide." Source: job_fz_fullpanel.py adds CONF975 to both the VaR node and the numerical ES integral at 2.5%; the displayed formula matches the implementation, and the reader's alternative (integrating a level-dependent shift) is not what the code does. No formula change.
2. Section 3.3 now carries \label{sec:conventions}; the two R50 "marginal under the convention of Section~\ref{sec:fwer}" pointers, which resolved to 3.2, now point to 3.3.
3. Section 5.2: "6--11%" -> "6--12%" (the 2023 and 2024 ratios round to 12%); "(DM 2.5)" for the bootstrapped paths -> "(DM 2.5, and 1.1 on non-overlapping dates)"; holdout sentence adds "the rerun behind Online Appendix Table OA.13 gives +0.52%, DM 0.09" beside the +0.49%, DM 0.07 of the earlier run. OA.13 note states that the two are the same purged pipeline with different learner seeds.
4. Figure 2 caption: "attains the lowest FZ0 at both regulatory levels" -> "attains the lowest FZ0 at 1% and is never significantly beaten at 2.5%", matching the abstract and 5.1.
5. Section 5.1. Before: "The calibration tests show that the residual-hybrid's tail is well calibrated." After: "On the exception and $Z_2$ diagnostics the residual-hybrid's tail is calibrated; the exceedance-residual test rejects it at 1% ($p=0.04$, Online Appendix Table OA.5)."
6. Day-one attribution. Introduction, before: "it also delivers forecasts for assets that have no return history at all, something no per-asset model, parametric or not, can do." After: "its characteristics-only variant also delivers a forecast for an asset that has no return history at all, something no per-asset model, parametric or not, can do." Conclusion, before: "Because the estimator pools its residual model across names, it also forecasts tail risk for assets with no history of their own, forecasting new listings from their first day as no per-asset model can." After: "Because the shape model is pooled across names, its characteristics-only amortized variant produces a forecast for a newly listed asset from its first day, and the residual-hybrid attaches once a scale filter is estimable (Section~\ref{sec:applications})." Abstract unchanged ("transfers to newly listed assets").
7. OA.9. Before: "The application the evidence supports is the opposite side, selling tail protection, where the model supplies tail control rather than alpha on the mean." After: "A possible application is the opposite side, selling tail protection with the model supplying tail control rather than alpha on the mean; the paper evaluates no prices, premia, costs or trading outcomes, so this is a direction and not a result."
8. Table OA.13 reset in \scriptsize with short column heads (was clipped at the right margin).

## Runs of 2026-09-24 (no text change yet; predictions in each script header, written before the run)

1. Learner feature ablation (job_feature_ablation.py, feature_ablation_results.json; Table 1 rows, 200 names, 221,600 asset-days). Top mk63-decile edge over GARCH-t: full learner +2.98% (DM 10.5); without absz5 and zstd21 +1.93% (DM 10.4); GARCH scale alone +1.63% (DM 11.0). Overall: +0.35%, +0.14%, +0.12%. Decile ordering unchanged in every variant; bulk deciles within -0.43% to +0.46%. Prediction P1 (scale alone keeps more than +1.5% with DM > 4, ordering intact) confirmed; P2 (removing the dispersion features changes the top-decile edge by under 0.5 points) not confirmed, the drop is 1.05 points. Reading: about 55% of the top-decile edge is available from the scale state alone, the trailing residual-dispersion features add about a third, and the score orders the edge whichever inputs the learner sees.
2. State-adaptive parametric benchmark (job_arcd_bench.py, arcd_bench_results.json). GARCH-t with nu_t = 2.1 + exp(h_t), h_t = c + a|z_{t-1}| + b h_{t-1}, fitted by ML per name on training residuals with the scale fixed; median a 0.11, b 0.64, median train log-likelihood gain over constant nu 2.85 (47% of names gain more than 3), nu_t 10th to 90th percentile 3.7 to 5.2. Over GARCH-t it gains +0.27% (DM 1.9) in the top mk63 decile and +0.10% (DM 3.8) overall; the learner keeps +2.72% (DM 9.7) over it in the top decile and +0.26% (DM 3.4) overall. Prediction (closes under a third of the gap, learner keeps more than +2.0%) confirmed: it closes 9%.
3. FRTB battery on the 200-name panel (job_frtb200.py, frtb_table_200_results.json; 221,600 rows, 1,108 dates). Ordering unchanged: residual-hybrid best (0.3676) and alone in the 90% MCS, EVT-tailed variant second (0.3680), GARCH-t and GJR-skew-t 0.3688, FHS variants 0.3689 to 0.3694, HS 0.3752, EWMA 0.3735; DM of the best against GARCH-t 6.3 (5.0 on 140 names), against GJR 5.6 (5.0). Prediction as written not met on levels (every row is about 0.012 higher because the 60 added names are more volatile), met on gaps (engine minus GARCH-t 0.0012 versus 0.0010). Pooled Kupiec at 1% now passes GARCH-t (p = 0.17), GJR (0.43) and FHS (0.21) and still rejects the residual-hybrid (0.0025, breach 1.06%) and the EVT variant (breach 0.88%); at 2.5% the pattern is as on 140 names.

## R52 (2026-09-24): ablation and adaptive-nu benchmark in the text

1. Section 4.1, new paragraph after "...and not as a diagnostic of shape alone.": "\paragraph{Two checks on the decomposition.} Neither the learner's inputs nor a richer parametric family accounts for the frontier. Retraining the Table 1 learner without its trailing residual-dispersion features leaves a top-decile edge of $+1.93\%$ (DM 10.4), and with the GARCH scale as its only input $+1.63\%$ (DM 11.0), with the decile ordering unchanged in both; the score is never a learner input, so about half of the top-decile edge is available from the scale state alone and the dispersion features add roughly a third (Online Appendix Table OA.14). A GARCH-$t$ whose degrees of freedom move with the lagged absolute residual, fitted per name by maximum likelihood, gains $+0.27\%$ (DM 1.9) over constant-$\nu$ GARCH-$t$ in the top decile, and the learner keeps $+2.72\%$ (DM 9.7) over it there (Online Appendix Table OA.15), so a state-adaptive shape within the parametric family does not reach the frontier." Sources: feature_ablation_results.json, arcd_bench_results.json.
2. Online Appendix: Table OA.14 (feature ablation) and Table OA.15 (adaptive-nu benchmark) added after OA.13; the sample-flow table is now OA.16. Table 3 stays on the 140-name panel; the 200-name rerun is on record in frtb_table_200_results.json and the README.

## R53 (2026-09-24): stale-reference sweep after R47--R52

Every hard-coded "Table OA.N" and "Figure OA.N" in the main text was mapped against the appendix numbering after the OA.13--OA.15 insertions (all resolve), every "Section N of the paper" and "equation (N) of the paper" in the appendix against the main text (all resolve). Three summary rows in Online Appendix Table OA.1 lagged the body:
1. "accuracy layer lowest at both levels" -> "accuracy layer lowest at 1%, never significantly beaten at 2.5%".
2. "CAViaR (strongest academic rival) & tie (both in 90% MCS) & DM 0.4" -> "SAV-CAViaR (shares the frontier) & tie on its common sample (both in 90% MCS); within a third of a percent on the frontier rows & DM 0.4 (Table 4 of the paper); -1.8 top decile, -1.6 overall, engine over CAViaR (Table OA.10)".
3. Ten-day row now carries the iterated-benchmark result and points to Table OA.13.
README (both repositories): the post-submission paragraph now quotes the current abstract wording and lists the September runs (ten-day diagnostics, feature ablation, adaptive-nu benchmark, FRTB on 200 names).
