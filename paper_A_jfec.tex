...
Unit-variance fat-tailed laws put more mass near the center as well as in the far tail, so the ordering of Proposition~\ref{prop:wedge} reverses at moderate levels: the unit-variance $t_3$ has a less extreme 5\% quantile than the normal but a more extreme 1\% quantile, with the sign flip at $\tau_c \approx 0.0179$. The deepest regulatory level $\tau=0.01$ sits below the crossover, where the fat tail is strictly more extreme, while $\tau=0.025$ sits just above it. Two design choices follow: whole-distribution scoring can rank a fat-tail-aware model below a thin-tailed one even when the tail is the decision-relevant region (Section~\ref{sec:notation}), the score dependence of \citet{gneitingranjan2011} and \citet{patton2020}; and the score of Section~\ref{sec:score} is used as a rank rather than a moment estimate.
\end{remark}
...
converges to it. This is the corollary of Table~\ref{tab:frontier}: the
...
envelope. The horizon comparison ends with a mixed verdict; the full
