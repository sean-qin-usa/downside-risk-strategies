...
\begin{abstract}
Parametric Value-at-Risk and Expected Shortfall forecasts fail in states
that can be flagged in real time. A per-asset, per-day score of
...
conditional shape. The forecasts come from an amortized semiparametric
estimator whose single pooled fit also covers newly listed assets with no
history, and it attains the lowest joint (VaR, ES) loss against standard
benchmarks.
\end{abstract}
