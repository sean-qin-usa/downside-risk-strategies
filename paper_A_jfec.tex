...
\begin{abstract}
Value-at-Risk and Expected Shortfall are usually built from a parametric
return distribution that is hard to beat on most days. We construct a
real-time score, from trailing GARCH-standardized residuals, that predicts
per asset and day when a distribution-free alternative wins, and decompose
that advantage: a jump-robust GARCH shows most of it is the filter's
variance overstatement after a shock, with a smaller significant part
genuine conditional shape. The gain concentrates in the score's top
decile; elsewhere the forecasts match. An amortized estimator attains the
lowest joint (VaR, ES) loss against standard benchmarks and prices newly
listed names from day one.
\end{abstract}
...
both directions, tying up capital against risk that never arrives or
leaving too little against the loss that does. Deciding when a more
flexible model is worth its added complexity is therefore a practical
question, and not only for the bank trading desks that Basel regulates:
asset managers, hedge funds, and any function that forecasts a conditional
tail face the same choice. Value-at-Risk (VaR) and Expected Shortfall
(ES), the forecasts at issue, summarize the downside risk of a trading
book: the lower-tail quantile of the conditional return distribution, and
the average loss beyond it. Three
...
\citep{zhang2024commonality}. Whether a comparable advantage exists for
the quantile learners studied here, whether it reaches the regulatory
tail, and on which days, is the question this paper takes up.
...
\citep{noldeziegel2017elicitability}; beating a vanilla GARCH on average
loss is accordingly not enough, though neither is it trivial, since bank
internal models have historically failed to beat a simple GARCH fitted
to their own profit and loss \citep{berkowitzobrien2002}. The GARCH
filter is the floor of this paper's comparisons rather than its target.

The advantage of a flexible model over this parametric benchmark is not
spread evenly across days, and that unevenness is the paper's subject. On the roughly ninety percent of asset-days (one name on one trading date) when residual-shape stress is low, the estimator we use and the parametric benchmark are statistically indistinguishable on forecast accuracy, so holding the flexible model carries no detectable penalty (Section~\ref{sec:frontier} reports the intervals). The gains are concentrated in the minority of days
when residual-shape stress is high. Locating that minority in advance,
from an observable signal, is the paper's central task. It is a
...
the density-forecast diagnostics of \citet{berkowitz2001}. What is new is
the use to which these diagnostics are put: computed on recent
residuals, they predict when distribution-free quantile methods improve
on parametric VaR/ES. Where residual-shape stress is high the flexible estimator has a large and significant advantage, and where it is low the advantage is statistically indistinguishable from zero in the design panel and a small positive in the point-in-time universe, with no detectable loss in either. The
score's content is an ordering of magnitudes, with the edge
concentrated in the top decile; there is no sharp threshold below which
it vanishes.

\medskip\noindent This paper makes three contributions. First, to the evaluation of VaR and ES forecasts and the comparison of competing conditional models, it introduces the misspecification frontier: a real-time, per-asset, per-day score that predicts when distribution-free quantiles beat a GARCH-$t$ benchmark, with significance from Diebold--Mariano tests on date-averaged loss differentials (the per-date DM of Section~\ref{sec:protocol}). The frontier is validated across four universes, namely CRSP US equities (top-decile edge $+2.98\%$, DM 10.5), foreign exchange (hyperinflation tier, pooled DM 4.12), twenty-six country indices (a developed-to-frontier-market gradient), and forty-three cross-asset instruments. A central contribution is to identify what that advantage is made of. Measured against a jump-robust GARCH that cannot overshoot its variance after a large shock, we show that most of it is the standard filter's post-outlier scale error, cleanly separated from a smaller but significant conditional-shape component that survives a robust scale (the top-decile $+2.98\%$ becomes $+0.3$--$0.5\%$, DM $2.5$--$4.7$). This attribution, drawn on a 200-name panel and tied to a live score, is new to the flexible-versus-parametric tail-risk literature. The Korea-2026 crash, which unfolded under circuit breakers and left residual kurtosis in its ordinary range, provides a falsification case that sharpens the claim: a price crash is not residual misspecification, and GARCH-$t$ wins there, as the frontier predicts.
...
machine-learning models pooled across stocks and document transfer to previously unseen names. That transfer is evidence of a shared volatility mechanism, which our amortized design also exploits. For the VaR comparison the antecedent is \citet{kuester2006}, and for machine-learning volatility forecasting \citet{bucci2020} and \citet{christensen2023ml}. Deep neural-network quantile regression applied directly to VaR is the recent comparison closest to our nonparametric stage \citep{chronopoulos2024}. The differences delimit this
...
calibration split behind the conformal overlay). In ongoing use each
...
This paper's organizing claim, that the nonparametric edge is concentrated where
the trailing residual-shape diagnostics are extreme, is an empirical finding with a structural motivation in the pinball loss. The results below are modest: a shape violation creates a quantile wedge, and regret is locally quadratic in that wedge. They motivate looking where trailing shape diagnostics are extreme, but they do not prove that $\mathrm{mk}_{63}$ (Section~\ref{sec:score}) estimates the wedge. That link is established empirically. The identity in Lemma~\ref{lem:regret} is the quantile-score divergence of \citet{gneiting2011}, in the mixture form of \citet{ehm2016}, and the lower bound in \eqref{eq:quadratic} is the self-calibration inequality of \citet{steinwart2011}. We restate both because Proposition~\ref{prop:wedge} uses the constants.
...
pays is \emph{locally quadratic in its quantile error}. The frontier claim then reduces to whether shape misspecification creates a quantile wedge at the risk levels that matter. For the paper's canonical case, a fixed Student-$t$ shape when the true local shape is fatter, the premise of GARCH-EVT \citep{mcneilfrey2000, francqzakoian2015} and the pattern in the horse race of \citet{kuester2006}, it must:
...
regulatory levels, because two features of the standardized problem
...
The hypothesis class of Stage 2 contains the constant-in-$s$ map $q(\tau)$, i.e., FHS \citep{baroneadesi1999fhs}. Empirical risk minimization over the larger class therefore attains in-sample pinball risk no worse than FHS by construction, and the out-of-sample comparisons of Section~\ref{sec:frtb} test whether that dominance survives estimation noise. It does. The same argument does not apply to CAViaR \citep{engle2004caviar}, so CAViaR is the one rival the estimator only ties.
\end{remark}
...
CRSP delisting returns where present. A name whose delisting return CRSP does not record ends at its last regular return, the omission \citet{shumway1997} shows is large and negative for performance-related delistings. The one delisting event in the
...
licensed subscriber. Table~\ref{tab:sampleflow} gives the sample flow.
...
and robustness checks: the calendar-time split, the pre-2007 cut, the
...
model beats the frozen parametric benchmark, and because the
residual-hybrid holds the parametric scale fixed, its advantage enters
through the residual quantile rather than the scale
(Section~\ref{sec:frtb} shows that on large capitalizations a
realized-variance scale absorbs much of this component).
...
to look ahead, the panel form of the overlap leakage that purging and embargo address \citep{lopezdeprado2018}. A per-asset split therefore
...
split, the convention for pooled equity learners \citep{gukellyxiu2020}, closes it. Each stage (each GARCH fit, the pooled learner, the GPD tail) is re-estimated on data strictly before 2020-01-01 and tested from that date for
each name simultaneously, so no training window shares a single
calendar day with any test window. The frontier survives this split: the top decile carries
...
leaves few calm cells, but the ordering is intact, with the top decile
roughly doubling each other cell, so the concentration is not a
...
A last construction tests the schedule the paper recommends. Because
the evaluated protocol freezes each stage once per split, the score
could in principle be measuring parameter staleness rather than shape.
The annual walk-forward removes that possibility: it refits the per-name GARCH on expanding data at each January-1 cutoff from 2020 to 2024, rebuilds the residuals and the score under the refit parameters, and retrains the pooled learner. Under this schedule the top decile carries $+2.09\%$ at per-date DM 4.43, still large and significant, while the overall edge is no longer detectable ($-0.18\%$, DM $-0.52$); refitting removes the average gap but not the concentration, so stale benchmark parameters do not produce the conditional result.

\subsection{Cross-sectional dependence, family-wise error, and the
universe rule}
\label{sec:fwer}

Each main test collapses the cross-section
...
asset-days. The per-date test is the pooled-average panel test of \citet{qutimmermannzhu2024}, whose simulations show that tests treating asset-days as independent over-reject under common-date dependence.
...
family-wise-error control \citep{romanowolf2005} over the full $3\times10$ grid, with adjusted $p$-values computed as in \citet{romanowolf2016}. The inputs
are studentized per-date mean edges per cell (the cell $t$-statistics below), a stationary bootstrap over dates \citep{politisromano1994}
that draws the same dates for every cell and so preserves cross-hypothesis dependence (mean block 10 days,
...
significance under familywise control. The main claims rest on
...
observation filter biased the reported edge downward. The construction still requires the symmetric fitting floors, so the
...
$\sqrt{K}$-sigma cap that keeps a single shock from propagating into the
variance) and so cannot overshoot. Against that benchmark the top-decile
...
Figure~OA.1 probes that alignment. The one-day lag gives the largest
...
the one-day lag ($+1.91\%$), and a one-day-ahead placebo gives $+1.92\%$,
so same-day sorting does not manufacture the edge.
...
rule the monitor uses) leaves $86\%$ of membership unchanged
and makes the sorting variable measurable at the forecast date, as the conditional test requires.
The top-bucket edge holds at $+2.45\%$ (DM 8.3), essentially the
full-sample figure, so the pooled-panel cutoff does not produce the
concentration. A within-date cross-sectional decile, which is not the
rule in use and forces $10\%$ occupancy on each date including calm
...
pooled learner. The natural candidate is per-name FHS with the residual
window reweighted, by a Gaussian kernel, toward days whose mk$_{63}$
resembles today's. On the full panel this reweighted FHS is
significantly worse than plain FHS, both overall (per-date DM $-11.3$)
and in the top decile (pinball 0.541 against 0.444), whereas the pooled
model beats plain FHS there by $2.7\%$ (DM 8.8). The deterioration is
consistent with a collapse of the effective sample, since sharp
univariate conditioning shrinks a name's usable history exactly when
today is unusual; pooling across names makes state-conditioning
affordable, as it does for volatility forecasts
\citep{bollerslev2018risk, zhang2024commonality}, because the amortized
model of Section~\ref{sec:gbcmethod} borrows shape information from
every name's stressed days.
...
loss panel against the full set of industry benchmarks (GARCH-$t$, FHS, EWMA, HS)
returns $T^{\mathrm{SPA}} = 0$, consistent $p = 1.00$: no rival's
advantage is positive, and each is individually worse with
$t \geq 5.8$. The score also needs no recalibration: holding the
...
era not used at any earlier stage of the analysis. The pre-specified kurtosis-component frontier, a simpler returns-space variant frozen in advance, replicates: the top-decile edge is $+0.89\%$ with per-date DM $2.16$ ($p=0.016$, 1{,}465 dates), deciles one through nine sit between $-0.18\%$ and $+0.28\%$, and the overall edge is $+0.02\%$. The residual-hybrid composite replicates as well (Figure~\ref{fig:holdout}): its top decile carries $+1.12\%$ at per-date DM $2.51$. This is the same threshold nonlinearity, attenuated in a larger-cap, pre-2014 universe.
...
rule never touches a forecast or a loss, but it could not have been
applied in real time on day one.
...
below, and the top-bin point edge slightly exceeds the within-sample sort's $+0.89\%$. The cell thins to $3.3\%$ occupancy in this lower-kurtosis era, however, and the per-date test loses power (DM 1.11, insensitive to Newey--West lags 5--44). A second real-time rule needs no design-era constant at all. An expanding pooled 90th-percentile threshold over strictly earlier dates (250-date burn-in) restores natural occupancy ($9.5\%$) and recovers the sort's result with significance ($+0.99\%$, DM 2.15, insensitive to Newey--West lags 5--44). The bottom bucket is $-0.18\%$ ($t=-1.4$). The conditional sort, the frozen constant, and the recursive rule agree on the size of the top-cell edge, and the recursive rule is the version that could have been run in real time from day one. The frozen thresholds are also tested on the point-in-time universe of Section~\ref{sec:fwer}, where they carry $+2.94\%$ at DM 6.6.
...
better and individually significant. Their sample residual kurtosis ($\sim$2000--3300)
sits two orders of magnitude above anything GARCH-$t$ was designed to
absorb, the signature of a pegged rate punctuated by discrete devaluations rather than of a fat-tailed continuous law. The country gradient tracks residual kurtosis
rather than the development label: frontier indices (kurt $\sim$10)
sit between well-behaved developed indices (kurt $\sim$5) and
hyperinflation FX on the same frontier. The Korea-2026 negative control separates the two: a price crash under daily price limits and circuit breakers, which censor the recorded return, is not residual misspecification. Korean residual kurtosis stays in the ordinary 4--13 range, and GARCH-$t$ significantly wins all twenty-three assets even inside the crisis window. What the frontier tracks is the residual shape; turbulence in prices alone does not produce an edge.
...
section rests on is aggregate rather than instrument-level: the crisis-tier FX
comparison (DM 4.12, $p\approx2\times10^{-5}$, below the Bonferroni
threshold for the 93-comparison family) and the kurtosis--edge gradients across universes. The tier statistic pools the seven pairs, and the per-asset tests for the peso and the pound are the primary evidence.

\paragraph{A frequency observation.} In the five daily crypto series examined here, GARCH wins throughout (zero
...
DM $p < 10^{-7}$. Sample density may be a further frontier dimension, since more
observations per unit of regime raise the resolvable residual-shape
signal, although the hourly comparison is also exposed to intraday periodicity that a daily-calibrated filter ignores. A systematic intraday map of the frontier has been specified in advance
and awaits tick-data access.

\subsection{Regime dependence and model selection}
\label{sec:gate}

In the amortized panel the absolute edge grows with turbulence. The
...
fourfold. Because pinball loss is scale-equivariant, part of that growth reflects the higher loss level in turbulent states rather than a larger relative edge, and both nonparametric estimators beat GARCH in
...
academic benchmark, produces a statistical tie: CAViaR 0.3461 vs residual-hybrid 0.3460 (DM 0.4, $p=0.34$), and the 90\% Model Confidence Set contains exactly these two. The CAViaR study's common sample differs slightly from Table~\ref{tab:frtb}'s, which is why the residual-hybrid's loss level is 0.3460 there against 0.3551 in the table. Rankings, not levels, are comparable across the two runs. The residual-hybrid thus ties CAViaR and significantly beats every model in banks' standard toolkit. The two also differ in what they deliver. SAV-CAViaR models one quantile level per fit, produces no ES (the FRTB metric) without an ad-hoc stage, and needs per-name history, so it cannot be applied to new listings; the residual-hybrid produces the full curve and extends to new listings. Pooled exception rejections are not counted against CAViaR, because pooled asset-day tests ignore cross-sectional dependence and overstate rejection for every model, the residual-hybrid included; the date-clustered restatement is the appropriate reading. Two textbook FHS variants, per-name train-constant and per-name rolling 500-day trailing, were run on the identical panel. Both lose to the residual-hybrid, by DM 5.2 and 5.7 (pinball 0.3562 and 0.3569 against 0.3551), and they bracket the pooled variant used in Table~\ref{tab:frtb}. A neural IQN used as the residual model loses to trees in its untuned form (0.3650 vs 0.3551, DM 13.3) and has a thin tail (breach$_{99}$ 3.2\%). The tail-aware, monotone, conformally recalibrated version narrows the gap (0.3638 vs 0.3632, DM 3.03) and repairs calibration (breach$_{99}$ 0.92\%). Trees remain the strongest tabular estimator, and the tuned IQN is competitive.
...
tests on its own (Online Appendix Figure~OA.2). Most of the concession reflects a frozen margin. The adaptive form of the shift \citep{gibbs2021aci}, updated daily from the panel breach frequency, shrinks the margin out of era (from $-0.35$ to $-0.10$, breach $1.8\%\to2.1\%$) and erases the overall concession. The adaptive variant ties GARCH-$t$ on 2.5\% FZ0 (DM $-0.3$). The top-decile cost survives either shift ($-4.8$ vs $-6.0$), the price of a margin where the bands are widest. The static shift also worsens per-name coverage (Kupiec$_{97.5}$ pass rate 47\%, against 72\% unshifted and 69\% adaptive), because over-coverage fails the test from the other side. Each ``lowest FZ0'' statement refers to the accuracy layer.
...
The multi-day results in this subsection come from a direct amortized model, a pooled quantile learner trained on $h$-day cumulative returns with GARCH-derived features; the residual-hybrid estimator of Algorithm~1 is a one-day architecture, and no ten-day claim in this paper attaches to it. Training
...
GARCH-standardized residuals, it is available \emph{ex ante}: a risk manager
computes today's score from today's data and uses it for tomorrow's
risk number. What the evidence does not support is day-level
model-switching. As Section~\ref{sec:gate} shows, the per-day oracle gap is unreachable (predicted-edge correlation $+0.16$), because which day a tail event lands is not forecastable from prior-day state, and a learned gate collapses to always-nonparametric. The edge is negligible over the calm $\sim$90\% of asset-days, where the lower composite deciles sit within noise of zero and the nonparametric and residual-hybrid family is statistically indistinguishable from the parametric benchmark (Section~\ref{sec:frontier}; failing to reject is not equivalence). It is concentrated in the top misspecification decile ($+3.0\%$ pinball, DM 10.5) and in the most turbulent volatility regimes (a $\sim$fourfold gap, Section~\ref{sec:gate}). The score's role is therefore to stratify. The
one setting in which it does drive a switch is the
...
residual-hybrid is a one-day, single-name VaR/ES forecaster of the kind that feeds an internal model. The FRTB capital metric itself is a ten-day, portfolio-level stressed ES that this paper does not produce. The estimator's EVT-tailed accuracy layer attains the lowest joint FZ0 loss against the standard benchmarks and passes the aggregate date-clustered exception test at 99\% and marginally at 97.5\%, with residual
cross-sectional heterogeneity in the per-name diagnostics. This paper makes no
capital-release claim. The predicted-vs-realized ES wedge conditions
on each model's own breach set and depends on the tail-integration rule (Table~\ref{tab:frtb}), so no defensible arithmetic converts it to capital; the formal VaR/ES ranking is the jointly consistent FZ0 comparison of Section~\ref{sec:frtb}. The narrower implication is that the ES-based component of an IMA charge scales with the ES forecast. Actual FRTB capital also involves liquidity-horizon adjustment, stressed-period ES on a reduced factor set, constrained aggregation, non-modellable risk factors, and desk-level eligibility through backtesting and P\&L attribution \citep{bcbs2019frtb}. No one-for-one mapping from a single-name ES forecast to desk capital exists, and multi-day capital economics would also be regime-dependent. The ten-day rerun on
