...
complexity is therefore a practical question. It is not only a question
for the bank trading desks that Basel regulates. Asset managers, hedge
funds, and any function that forecasts a conditional tail face the same
choice. Value-at-Risk (VaR) and Expected Shortfall
...
loss is accordingly not enough. Neither is it trivial, because bank
...
score's content is an ordering of magnitudes. The edge concentrates in
the top decile, and there is no sharp threshold below which it vanishes.
...
\paragraph{Timing diagnostics.} The score uses residuals through day
$t-1$ paired with the day-$t$ loss; Online Appendix Figure~OA.1 probes
that alignment. The one-day lag gives the largest top-decile edge in this
returns-space diagnostic ($+2.69\%$, DM 6.5) and a five-day-old score
still retains $+1.67\%$ (DM 6.4), whereas a contaminated same-day window
and a one-day-ahead placebo both yield less ($+1.91\%$, $+1.92\%$), so
same-day sorting does not manufacture the edge.

\paragraph{Non-anticipating decile boundaries.} The sort forms deciles
once over the pooled test panel, so membership draws on the full-sample
distribution of mk$_{63}$. Re-forming the $90$th-percentile cutoff at
each date from only strictly earlier mk$_{63}$ ($250$-day burn-in, the
rule the monitor uses) makes the sorting variable measurable at the
forecast date and leaves $86\%$ of membership unchanged; the top-bucket
edge holds at $+2.45\%$ (DM 8.3), so the pooled-panel cutoff does not
produce the concentration. A within-date cross-sectional decile, which
forces $10\%$ occupancy on every date including calm cross-sections,
instead dilutes the edge to $+0.26\%$ (DM 2.2): the frontier ranks the
absolute level of residual misspecification, not within-day position.
...
pooled learner. Per-name FHS with its residual window Gaussian-reweighted
toward days whose mk$_{63}$ resembles today's is significantly
\emph{worse} than plain FHS, both overall (per-date DM $-11.3$) and in
the top decile (pinball 0.541 against 0.444), where the pooled model
instead beats plain FHS by $2.7\%$ (DM 8.8). Sharp univariate
conditioning collapses a name's effective sample exactly when today is
unusual; pooling across names makes state-conditioning affordable
\citep{bollerslev2018risk, zhang2024commonality}, because the amortized
model of Section~\ref{sec:gbcmethod} borrows shape information from every
name's stressed days.

\paragraph{Robustness of the score.} A double sort into quintiles of
mk$_{63}$ and of trailing volatility (Online Appendix Table~OA.2) shows
the two ingredients interacting: high volatility without a shape break
carries no edge and a shape break in the calmest regime almost none
($-0.10\%$, DM 0.7), while the product state carries $+2.33\%$ (DM 6.4).
A superior-predictive-ability test \citep{hansen2005spa} against the
industry benchmarks (GARCH-$t$, FHS, EWMA, HS) returns
$T^{\mathrm{SPA}}=0$ ($p=1.00$; each rival worse with $t\ge 5.8$). The
score needs no recalibration: with the design-era threshold held fixed
the top-decile edge is positive in every test year from 2020 to 2024
($+1.7\%$ to $+3.4\%$) despite an upward drift in the cross-sectional
90th percentile of mk$_{63}$. It is a slow state, so monitoring costs
little: a median name enters the top decile less than once a year (0.68
entries) and a median episode lasts 24 trading days.
