...
\begin{abstract}
We forecast Value-at-Risk and Expected Shortfall with an amortized
semiparametric estimator that keeps the GARCH volatility scale and learns
the tail shape nonparametrically. The same fit forecasts newly listed
assets with no history, evidence of a tail-shape mechanism shared across
stocks. A real-time score of residual-shape stress flags when the flexible
tail wins, an edge large and significant in its top decile and negligible
elsewhere. A jump-robust refit traces most of it to variance overstatement
after a shock, and a smaller but significant part to genuine conditional
shape. The estimator attains the lowest joint (VaR, ES) loss against
standard benchmarks.
\end{abstract}
