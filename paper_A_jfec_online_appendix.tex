...
Single asset, top misspecification decile & Nonparametric ($+2.98\%$, DM 10.5) & Section~5 of the paper \\
...
This section collects the constructions summarized in Section~4 of the paper.
...
\end{tikzpicture}
\caption{The residual-hybrid estimator as a pipeline (Section~3 of the paper). Solid arrows carry data; the dashed top arrow carries the conditional scale $\widehat\sigma_t$ that rescales the finished residual quantile curve. Dashed boxes are inputs shared across assets (the pooled panel that trains the amortized shape model) or held out from training (the calibration split behind the conformal overlay).}
\label{fig:pipeline}
\end{figure}
...
\centering\small
\begin{threeparttable}
\caption{Sample flow across evaluation panels (Section~4 of the paper).}
