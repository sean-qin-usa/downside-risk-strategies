...
\usepackage{amsmath,amssymb,amsthm,graphicx,booktabs,multirow,threeparttable,setspace}
...
(iv)~Act by the paper: top decile $\Rightarrow$ the
...
the misspecification score equation~(12) of the paper as the model-risk monitor ---
...
\midrule
Point quantile accuracy, tabular state & GBM (trees) & \SSection~3.2 of the paper \\
Sampling, amortized posteriors, uncertainty & Neural IQN (only entrant) & Paper~B \\
Single asset, calm ($\sim$90\% of asset-days) & GARCH-$t$ & \SSection~5 of the paper \\
Single asset, top misspecification decile & Nonparametric ($+2.7\%$, DM 6.5) & \SSection~5 of the paper \\
Amortized cross-name panel, any regime & Nonparametric always; no gate & \SSection~5.4 of the paper \\
Regulatory ES with a $\ge$GARCH floor & Residual-hybrid $+$ EVT $+$ conformal & \SSection~6 of the paper \\
...
shape). Three results. First, the Lemma~1 of the paper identity
...
split, as Proposition~1 of the paper promises. Third --- the
...
in a static $\nu$. For the empirical sections this means the real-data top-decile edge of Section~5 of the paper
