...
is jointly elicitable \citep{fissler2016}, ES admits direct backtests, and the
main text adds two: the McNeil--Frey (2000) exceedance-residual bootstrap, the
mean standardized loss beyond VaR and zero under a correct ES
...
panel (140 CRSP names, per-name GARCH-$t$ fit on the first $60\%$, evaluated on
the held-out $40\%$; $155{,}120$ pooled test observations per model). Two
readings stand out. Fat tails are necessary: the Gaussian-innovation filter is
worst on every ES metric, its mean exceedance residual reaching $-0.79$ standard
deviations at the $1\%$ level ($Z_2=-1.07$). And the EVT-tailed engine is the
best-calibrated entrant: it carries the smallest $Z_2$ at both levels, its ES
passes the McNeil--Frey test at $2.5\%$ (mean exceedance residual $-0.02$,
$p=0.46$), and it is the only entrant whose $1\%$ exception count passes Kupiec.
The honest limit is the deep tail. At $1\%$ every model's mean exceedance
residual is significantly negative on this large-capitalization panel, the
engine included; and, as with the pooled Kupiec test at this sample size, the
$Z_2$ $p$-values are hypersensitive, so the statistic magnitudes and the
McNeil--Frey ordering, not the pooled $p$-values, are the informative reading.
