...
\subsection*{The intraday ceiling: a realized-measure comparator}\label{sec:rgarch-oa}
The main-text battery reads only daily returns, the regime the estimator is
built for. To place it against a method that reads intraday data, we build
five-minute realized measures from TAQ for thirty large-capitalization U.S.\
equities over 2014--2024 (realized variance is the sum of squared five-minute
log-returns, cleaned by regular sale conditions, a daily price band, and a
$20\%$ return clip), and fit a Realized GARCH
\citep{hansen2012realized}, the conditional variance driven by lagged realized
variance, $\log h_t=\omega+\beta\log h_{t-1}+\gamma\log \mathrm{RV}_{t-1}$,
estimated by Gaussian QMLE on the training split. We give the realized model
both a parametric-$t$ tail and, for a matched comparison, the paper's own EVT
tail on its standardized residuals, and score every model on the identical
$(\VaR,\ES)$ FZ0 loss and date-clustered Diebold--Mariano test used in the main
text. Table~\ref{tab:rgarch} reports the result on 1{,}108 common test dates.
The realized-measure model lowers FZ0 well below the daily-data accuracy core
at both levels (DM $6.0$ at 2.5\%, $4.5$ at 1\%), and is at least as well
calibrated. The advantage is entirely the conditional scale: the two tail
choices are within noise of each other, so what the daily filter cannot match
is the realized \emph{scale}, not the tail treatment. The estimator's own
contribution, the amortized pooled shape model and the misspecification score,
is orthogonal to this scale channel and unavailable to the realized model in
the day-one and no-history regimes; a realized-hybrid that carries the
estimator's tail and score on a realized-variance scale is the natural
combination.
...
\end{tabular}
\begin{tablenotes}\footnotesize
\item Returns are CRSP daily; the intersection with the realized panel is
common across models. The daily core is the paper's GARCH scale with the EVT
tail (no conformal shift). Source: \texttt{rgarch\_bench\_results.json}.
\end{tablenotes}
\end{threeparttable}
\end{table}
