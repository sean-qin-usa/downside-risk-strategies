...
"=== jfec_sync14 $(Get-Date -Format o) ===" | Out-File $log -Encoding utf8
Set-Location $PROJ
...
git commit -m "JFEC-corpus structural pass: intro summary table moved to online appendix; descriptive section headings; reading copy 39pp" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>" -m "Claude-Session: https://claude.ai/code/session_01PaysugvHD2tgCJvXLSscWa" 2>&1 | Select-Object -First 2 | Out-File $log -Append -Encoding utf8
...
git add -A 2>&1 | Out-Null
git commit -m "Intro table relocated to OA; descriptive headings (VaR and ES forecast evaluation; regime dependence and model selection)" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>" -m "Claude-Session: https://claude.ai/code/session_01PaysugvHD2tgCJvXLSscWa" 2>&1 | Select-Object -First 2 | Out-File $log -Append -Encoding utf8
