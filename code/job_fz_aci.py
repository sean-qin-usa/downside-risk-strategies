...
# Wave-9 rerun: static warm start uses the exact ceil((n+1)tau) order statistic (Prop. 1).
...
# same engine, same GPD tail as the pre-committed fz_fullpanel run.
...
s975=CALzc['z'].values-ZQcal; C0=conf_ostat(s975,A)   # exact ceil((n+1)tau) order statistic (Prop. 1)
