# job_composite_profiles.py (job_composite.py plus per-signal decile profiles) -- EVALUATE THE EXACT DEPLOYED SCORE [R20 #1 FATAL].
...
OUT={'note':('Exact Algorithm-OA.2 composite score = rank of the MAX of {mk63, |sk63|, jump5}. '
 ...
 'signal_decile_profiles':{'mk63':{int(dd0):edge(rk_mk==dd0) for dd0 in range(1,11)},'skew63':{int(dd0):edge(rk_sk==dd0) for dd0 in range(1,11)},'jump5':{int(dd0):edge(rk_jp==dd0) for dd0 in range(1,11)}},
 'overall':edge(np.ones(len(Y),bool))}
json.dump(OUT,open(os.path.join(P,"composite_profiles_results.json"),"w"),indent=2)
