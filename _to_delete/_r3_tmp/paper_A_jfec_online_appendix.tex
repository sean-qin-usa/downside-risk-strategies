...
Fix $\tau$ below the crossover level of the paper's crossover remark, so
that by Proposition~2 of the paper $Q_G(\tau)$ is strictly less extreme
than $Q_{F_\delta}(\tau)$ and the gap $w(\delta)=|Q_G(\tau)-Q_{F_\delta}(\tau)|$
...
the tail ordering of Proposition~2 pushes $Q_{F_\delta}(\tau)$
monotonically outward while $Q_G(\tau)$ is fixed, so $w'(\delta)>0$. By
Lemma~1 of the paper (the regret identity), the benchmark's level-$\tau$
excess loss under local truth $F_\delta$ is
$A_\tau(\delta)=\tfrac12 f_\delta(Q_{F_\delta}(\tau))\,w(\delta)^2+o(w(\delta)^2)$,
a strictly increasing function of $w(\delta)$ and hence of $\delta$ near
$\delta=0$; the oracle attains $Q_{F_\delta}(\tau)$ and carries no
first-order regret, and the nonparametric estimator attains it up to
$O_p(n^{-1/2})$, which is $o(1)$ in the advantage as the sample grows.
Finally, the trailing-window sample excess kurtosis $S_t$ is the fourth
standardized sample moment minus three; by the law of large numbers
$S_t\xrightarrow{p}\kappa(\delta)$ whenever $\mathbb{E}[z^4]<\infty$ (so
for $t_\nu$ with $\nu>4$; the sampling variance of $S_t$ is finite for
$\nu>8$). Composing the monotone maps
...
aggregate pinball advantage is the $\tau$-weighted sum
$\sum_\tau \omega_\tau A_\tau(\delta)$, dominated by the tail levels for
which the monotonicity holds, and the joint $(\mathrm{VaR},\mathrm{ES})$
object is a level-by-level functional of the same quantile curve; both
inherit the ordering. \hfill$\square$
