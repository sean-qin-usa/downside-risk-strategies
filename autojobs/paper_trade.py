...
def sim(fill="mid"):
    ...
    bl=pd.DataFrame(blotter); bl.to_csv(os.path.join(P,"paper_blotter.csv"),index=False)
    traded=bl.groupby('month')['pnl_pctK'].mean()/1e4                      # traded months only
    # portfolio series: skipped months earn ~0 (cash-secured capital sits idle) -> honest deployed-capital Sharpe
    allmix=sorted(set(traded.index)|set(skipped))
    port=pd.Series(0.0,index=pd.PeriodIndex(allmix,freq='M')); port.loc[traded.index]=traded.values
    sr=lambda x: round(float(x.mean()/x.std()*np.sqrt(12)),2) if x.std()>0 else None
    summ=dict(fill=fill, months_traded=int(bl.month.nunique()), months_skipped=len(set(skipped)),
              n_tickets=len(bl), avg_pnl_bp=round(float(bl.pnl_pctK.mean()),1),
              portfolio_SR=