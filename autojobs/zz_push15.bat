...
set LOG=%PROJ%\zz_push15_log.txt
cd /d "%PROJ%"
echo === lock sweep === > "%LOG%"
...
git add -A > nul 2>> "%LOG%"
git commit -m "De-AI heading sweep (JFEC-convention noun headings); long abstract restored to master, 100w version stored for the JFEC skin" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>" -m "Claude-Session: https://claude.ai/code/session_01PaysugvHD2tgCJvXLSscWa" >> "%LOG%" 2>&1
