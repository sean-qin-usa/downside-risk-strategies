...
"=== jfec_sync16 $(Get-Date -Format o) ===" | Out-File $log -Encoding utf8
Set-Location $PROJ
...
git commit -m "Wave-10: coherent Q* made canonical in frtb_table and fz_fullpanel (VaR+ES from one min-envelope curve; FZ0 edge grew to DM 4.8/5.1); authorship Sean Qin + Wenxin Jiang; necessary-not-sufficient logic fix; IQN/lemma/normalizer fixes; deleted meta-sentence; em-dashes 220 to 37/32/0; self-referential and marketing language removed" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>" -m "Claude-Session: https://claude.ai/code/session_01PaysugvHD2tgCJvXLSscWa" 2>&1 | Select-Object -First 2 | Out-File $log -Append -Encoding utf8
...
git add -A 2>&1 | Out-Null
git commit -m "Canonical coherent-ES reruns; two-author byline; editorial de-AI pass" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>" -m "Claude-Session: https://claude.ai/code/session_01PaysugvHD2tgCJvXLSscWa" 2>&1 | Select-Object -First 2 | Out-File $log -Append -Encoding utf8
