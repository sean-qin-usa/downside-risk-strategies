...
"=== jfec_sync $(Get-Date -Format o) ===" | Out-File $log -Encoding utf8
Set-Location $PROJ
git add -A 2>&1 | Out-Null
git commit -m "JFEC submission build: 41pp double-spaced main (97-word abstract) + 13pp online appendix" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>" -m "Claude-Session: https://claude.ai/code/session_01PaysugvHD2tgCJvXLSscWa" 2>&1 | Select-Object -First 2 | Out-File $log -Append -Encoding utf8
...
git add -A 2>&1 | Out-Null
git commit -m "Add JFEC submission build under submission/" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>" -m "Claude-Session: https://claude.ai/code/session_01PaysugvHD2tgCJvXLSscWa" 2>&1 | Select-Object -First 2 | Out-File $log -Append -Encoding utf8
