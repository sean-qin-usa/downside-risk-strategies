...
A real-time score for when nonparametric models beat industry-standard VaR and ES, and a deployable amortized engine that captures the difference.

**Current live draft: [paper_A_frontier.pdf](paper_A_frontier.pdf)** ([LaTeX source](paper_A_frontier.tex) Â· [bibliography](refs_v3.bib))
...
A single measurable quantity â€” the excess kurtosis and asymmetry of recent GARCH-standardized residuals â€” governs when flexible-shape quantile methods beat parametric VaR/ES. Where that misspecification score is high, an amortized nonparametric engine wins decisively (top decile +2.7% pinball, DM 6.5; replicated on an untouched 2000â€“2013 holdout under registered predictions); where it is low, the engine and the parametric model tie. The engine passes Kupiec and Christoffersen backtests at both regulatory levels through a 2008-crisis test window and attains the lowest joint (VaR, ES) FZ0 score on the full panel.
...
| `results/` | Derived statistics as JSON â€” every number in the paper traces to one of these |
